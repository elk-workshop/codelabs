...
summary: Elastic  Security 实战工作坊(中等)
id: elastic-security-on-qq
...
# Elastic Security 实战工作坊 (中等) - 腾讯云版
<!-- ------------------------ -->
...
本课程使用的 Elastic Stack 安装包，示例配置文件都可以在腾讯云的镜像站点下载 https://mirrors.cloud.tencent.com/elasticstack/7.x/yum/7.10.1/。
...
* kibana-7.10.1-x86_64.rpm



### 创建一个 Elasticsearch 集群

登录腾讯云控制台，搜索 Elasticsearch Service 产品，进入该产品控制台，点击 “新建” 集群按钮。图中的版本是“7.5.1”，本教程使用的版本是 **“7.10.1”** ，请确保使用此版本。
...
在上面的 Elasticsearch 集群相同的网段中创建一个 CentOS 7 虚拟机。
...
在浏览器里测试登录 Kibana 的公网访问地址 [ https://es-byuqwb9l.kibana.tencentelasticsearch.com:5601](https://es-byuqwb9l.kibana.tencentelasticsearch.com:5601/)，使用 elastic 用户名和密码 Security@1234#。点击主页上的“安全”的链接，你应该看到如下的界面。
...
点击 Kibana 左上角菜单中点击 “堆栈监控”，自监控功能是默认启用的（监控数据驻留在本集群内部），结果如下所示。
...
```sh
yum install -y https://mirrors.cloud.tencent.com/elasticstack/7.x/yum/7.10.1/auditbeat-7.10.1-x86_64.rpm
```
...
auditbeat setup -e \
  ...
  -E output.elasticsearch.hosts=['192.168.0.4:9200'] \
  -E output.elasticsearch.username=elastic \
  -E output.elasticsearch.password=Security@1234#\
  -E setup.kibana.host=http://es-byuqwb9l.internal.kibana.tencentelasticsearch.com:5601
```

待以上文件执行成功以后，在 Kibana 中，点击Dashboard，浏览 Auditbeat 的仪表板。点击名为 “[Auditbeat System] System Overview ECS”的仪表板。
...
```sh
cp sec-4.yml /etc/auditbeat/auditbeat.yml
...
yum install -y https://mirrors.cloud.tencent.com/elasticstack/7.x/yum/7.10.1/packetbeat-7.10.1-x86_64.rpm
```
...
```sh
cp /vagrant/sec-5.yml /etc/packetbeat/packetbeat.yml
...
```sh
yum install -y https://mirrors.cloud.tencent.com/elasticstack/7.x/yum/7.10.1/filebeat-7.10.1-x86_64.rpm
```
...
将以上示例配置文件的内容修改为名为 **sec-6.yml** 的配置文件。

用下面的命令更新 Filebeat 默认的配置文件，测试文件的可用性，并启动服务。

```sh
cp sec-6.yml /etc/filebeat/filebeat.yml
...
![2021-04-15_17-45-20](images/2021-04-15_17-45-20.png)
...
当某个时间线的分析结果被判定为安全事故之后，就需要根据它来创建一个故障调查案例，这是 SIEM 的常用场景。点击时间线右上角的齿轮图标后，在菜单中选择 “附加到新案例” 选项，进入新建案例界面，如下图所示：

![2021-04-15_17-50-32](images/2021-04-15_17-50-32.png)

创建新的调查案例。

![2021-04-15_17-53-13](images/2021-04-15_17-53-13.png)
...
点击 Kibana 首页上的“开发工具”，这里是使用目前 EQL 事件查询语言的界面。从 7.12.0 版本之后，EQL 的图形化查询界面就移植到了 “时间线” 分析的 界面中。

![2021-04-15_20-21-13](images/2021-04-15_20-21-13.png)
...
2. auditbeat-* ： 这个索引模式是存储上面的序列测试事件的索引。

3. 点击 “小三角” 图标触发查询

4. 在右侧的搜索结果中，搜索关键字 _id，每个单条登录事件中有里有两个字段还有 _id， 浏览下面的三条事件的详情 。
...
- 可以将最后一条的 success 删掉，搜索一下，思考结果是什么意思。
- 可以将最后一条的 success 替换成 failure ，搜索一下，思考搜索结果是什么意思。
...
Negative
: 注意：EQL 是专门为安全狩猎、为复杂性较高的安全分析而生的；可以直接通过 Elasticsearch 的 API调用 ` /_eql/` 完成所有搜索功能，这也是和其它系统集成的接口。

## 安全威胁狩猎

Duration: 10

Elastic Security 7.10增加了检测规则，并扩大了针对Windows主机的MITRE ATT&CK®覆盖范围。
...
2. 点击 “**管理检测规则**” 按钮。
3. 点击 “**加载 Elastic 预构建规则和时间线模板**” 按钮。
4. 等待 2 分钟左右，就可以看到 7.10 版本内置的 315条规则的，规则清单。
5. 点击标记，输入并选择 Linux，这筛选出来了 53 条规则。
6. 点击页面左下角的 每页行数 链接，选择 100 ，浏览所有规则名称清单。
7. 点击表格左上角，第一列上面的，全选所有规则的 勾选框 ☑️ ，选中所有规则。
8. 点击 “批处理操作” 链接中的 “激活所选” 选项，激活了这些规则。这个步骤可能会持续 3~5 分钟。
...
在虚拟机中，如果你的虚拟机只有一个网卡，而且名字不是默认的 eth0 ，则需要修改下面的参数文件。
...
查看 Elastic Security 的首页，在首页上应该可以看到，来自于 Suricata 的数据。
